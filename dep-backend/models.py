...
class User(db.Model):
    ...
    standingEarnedLeave: Mapped[Optional[int]]
    ...
    last_notification_check : Mapped[Optional[datetime]]
    ...
    def __init__(self, json):
        ...
        self.standingEarnedLeave = json.get('standingEarnedLeave',30)
        self.ltcInfos = []
        self.last_notification_check= json['last_notification_check']
    ...
    def json(self):
        return {
                ...
                "role": self.role.json(),
                "standingEarnedLeave": self.standingEarnedLeave,
...
class TAInfo(db.Model):
    ...
    lastForwardDate: Mapped[datetime]= mapped_column(nullable=True)
    blockYear : Mapped[Optional[int]]
    OtherThanHometown : Mapped[Optional[str]]
    GovtOffice : Mapped[Optional[str]]
    # stageCurrent: Mapped[int]


    def __init__(self, userId, ltcId, journeyDetails, certificate):
        self.userId = userId
        self.ltcId = ltcId
        self.journeyDetails = [JourneyDetail(journeyDet) for journeyDet in journeyDetails]
        self.stageCurrent = 1
        self.fillDate = datetime.now()
        self.blockYear = certificate.get('blockYear')
        self.OtherThanHometown = certificate.get('OtherThanHometown')
        self.GovtOffice = certificate.get('GovtOffice')
    ...
    def json(self):
        return {
                ...
                "receipts": [r.id for r in self.receipts],
                "blockYear": self.blockYear,
                "OtherThanHometown": self.OtherThanHometown,
                "GovtOffice": self.GovtOffice,
                }
